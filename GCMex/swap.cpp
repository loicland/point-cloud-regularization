    ...
    )
{
  ...
  enum {IN_CLASS=0,IN_UNARY,IN_PAIRWISE,IN_LABELCOST,IN_MAXITE,IN_NEIGH} ;
   ...
   ***************************************************************************/
  if (nin != 6)
    mexErrMsgTxt("Six arguments are required.");
  if (nout > 3)
  ...
  int *    maxIte  = (int*)mxGetPr(in[IN_MAXITE]);
  bool* neighbors  = (bool*   )mxGetData(in[IN_NEIGH]);
