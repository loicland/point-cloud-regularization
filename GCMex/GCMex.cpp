    ...
    )
{
  ...
  enum {IN_CLASS=0,IN_UNARY,IN_PAIRWISE,IN_LABELCOST,IN_EXPANSION,MAXITE} ;
   ...
   * ERROR CHECKING
   ***************************************************************************/
  if (nin != 4 && nin != 5 && nin !=6 && nin !=8)
    mexErrMsgTxt("Four or five or six arguments are required.");
  if (nin == 5)
  ...
  int *    maxIte   = (int*)mxGetPr(in[MAXITE]);
  ...
  try {
     ...
     */
    printf("maximum iteration set to %i\n",maxIte[0]);
    if(expansion)
    {
      gc->expansion(maxIte[0]);
    }
    else 
    {
      gc->swap(maxIte[0]);
