...
GCoptimization::EnergyType GCoptimization::giveSmoothEnergy()
{
	...
	if (readyToOptimise())
    {
        //printf("smooth E : %f", (this->*m_giveSmoothEnergyInternal)());
...
GCoptimization::EnergyType GCoptimization::giveDataEnergy()
{
	if (readyToOptimise())
    {
        //printf("data E : %f", (this->*m_giveDataEnergyInternal)());
...
GCoptimization::EnergyType GCoptimization::compute_energy()
{
	if (readyToOptimise())
    {
     EnergyType smooth = (this->*m_giveSmoothEnergyInternal)();
     EnergyType data   = (this->*m_giveDataEnergyInternal)();
     EnergyType total  = smooth + data;
     return (1*(this->*m_giveDataEnergyInternal)() + 1*(this->*m_giveSmoothEnergyInternal)());
    }
	else{ 
		handleError("Not ready to optimize yet. Set up data and smooth costs first");
		return(0);
...
GCoptimization::EnergyType GCoptimization::swap(int max_num_iterations)//bool* neighbors, 
{
	...
	{
		old_energy = new_energy;
		new_energy = oneSwapIteration();//(neighbors);
		//break;
		curr_cycle++;	
	}
...
GCoptimization::EnergyType GCoptimization::oneSwapIteration()//bool* neighbors)
{
	LabelID next,next1;
